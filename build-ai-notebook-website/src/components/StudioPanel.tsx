...
import { FileText, HelpCircle, BookOpen, Clock, Briefcase, Sparkles, Pin, PinOff, Trash2, ChevronDown, ChevronRight, Headphones, Loader2, Wand2, Eye } from 'lucide-react';
...
export default function StudioPanel({ sources, notes, onAddNote, onTogglePin, onDeleteNote }: Props) {
  ...
  const doAudio=()=>{setAGen(true);setTimeout(()=>{setAGen(false);onAddNote({title:'🎧 Audio Overview',content:`# Audio Overview\n\n*Podcast-style overview.*\n\n${sources.filter(s=>s.selected).map(s=>`**${s.title}**: ${s.content.split('\n\n')[0]}`).join('\n\n')}\n\n---\n\n*Generates real audio in production.*`,type:'summary'})},2000)};
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-2.5">
        <div className="mb-4"><button onClick={doAudio} disabled={!sc||aGen} className="shine group relative w-full overflow-hidden rounded-2xl sB bg-gradient-to-r from-[#D4AF61]/[0.06] to-[#B4B8C8]/[0.04] p-4 transition-all duration-500 hover:from-[#D4AF61]/[0.1] disabled:opacity-35 active:scale-[0.98]">
          ...
          <div className="text-left"><p className="text-[11px] font-semibold text-white/90">Audio Overview</p><p className="text-[9px] text-[#8888A0]">{aGen?'Creating…':'Podcast-style summary'}</p></div></div>
